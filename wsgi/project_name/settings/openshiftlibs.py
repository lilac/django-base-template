...
#  }
def openshift_secure(default_key, secure_function = 'make_secure_key'):
    ...
    if my_token is not None:
        ...
        sha = hashlib.sha256(my_token).hexdigest()
        # Pass a dictionary so we can add stuff without breaking existing calls
        vals = { 'hash': sha, 'original': default_key }
        ...
        if secure_function is not None:
            ...
            if not supplied_function:
                ...
                secure_key = supplied_function(vals)
    else:
